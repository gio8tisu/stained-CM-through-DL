...
def main_unet(opt):
    # Create sample and checkpoint directories
    images_dir = 'images_unet/%s' % opt.dataset_name
    saved_dir = 'saved_models_unet/%s' % opt.dataset_name
    ...
    if opt.epoch != 0:
        # Load pretrained models
        G_AB.load_state_dict(torch.load(saved_dir))
        G_BA.load_state_dict(torch.load(saved_dir))
        D_A.load_state_dict(torch.load(saved_dir))
        D_B.load_state_dict(torch.load(saved_dir))
    else:
        # Initialize weights
        G_AB.apply(weights_init_normal)
        G_BA.apply(weights_init_normal)
    ...
    def sample_images(batches_done):
        ...
        img_sample = torch.cat((real_A.data, fake_B.data,
                                real_B.data, fake_A.data), 0)
        save_image(img_sample, images_dir, nrow=4, normalize=True)
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        ...
        if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0:
            # Save model checkpoints
            torch.save(G_AB.state_dict(), saved_dir)
            torch.save(G_BA.state_dict(), saved_dir)
            torch.save(D_A.state_dict(), saved_dir)
            torch.save(D_B.state_dict(), saved_dir)
